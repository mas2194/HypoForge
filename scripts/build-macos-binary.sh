...
echo "=== Building macOS Standalone Executables for my_harness ==="
...
"$ROOT_DIR/node_modules/.bin/esbuild" "$ROOT_DIR/src/bin.ts" \
  ...
  --target=node22 \
  --format=cjs \
  --banner:js="const import_meta_url = require('url').pathToFileURL(__filename).href; process.env.HARNESS_CLI = '1';" \
...
cp "$NODE_ARM64" "$RELEASE_DIR/my_harness-darwin-arm64"
codesign --remove-signature "$RELEASE_DIR/my_harness-darwin-arm64" 2>/dev/null || true
chmod 755 "$RELEASE_DIR/my_harness-darwin-arm64"
"$ROOT_DIR/node_modules/.bin/postject" "$RELEASE_DIR/my_harness-darwin-arm64" \
  NODE_SEA_BLOB "$DIST_DIR/sea-prep.blob" \
  --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2 \
  --macho-segment-name NODE_SEA
codesign -f -s - "$RELEASE_DIR/my_harness-darwin-arm64"

# Build darwin-x64
echo "    -> Building darwin-x64..."
arch -x86_64 "$NODE_X64" --experimental-sea-config "$DIST_DIR/sea-config.json"
cp "$NODE_X64" "$RELEASE_DIR/my_harness-darwin-x64"
codesign --remove-signature "$RELEASE_DIR/my_harness-darwin-x64" 2>/dev/null || true
chmod 755 "$RELEASE_DIR/my_harness-darwin-x64"
"$ROOT_DIR/node_modules/.bin/postject" "$RELEASE_DIR/my_harness-darwin-x64" \
  NODE_SEA_BLOB "$DIST_DIR/sea-prep.blob" \
  --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2 \
  --macho-segment-name NODE_SEA
codesign -f -s - "$RELEASE_DIR/my_harness-darwin-x64"

# Build universal binary using lipo
echo "    -> Creating universal binary (arm64 + x86_64)..."
lipo -create "$RELEASE_DIR/my_harness-darwin-arm64" "$RELEASE_DIR/my_harness-darwin-x64" \
  -output "$RELEASE_DIR/my_harness-darwin-universal"
codesign -f -s - "$RELEASE_DIR/my_harness-darwin-universal"

# Default executable link
cp -f "$RELEASE_DIR/my_harness-darwin-universal" "$RELEASE_DIR/my_harness"

# 5. Verify binaries
echo "==> [5/6] Verifying built executables..."
file "$RELEASE_DIR/my_harness-darwin-arm64"
file "$RELEASE_DIR/my_harness-darwin-x64"
file "$RELEASE_DIR/my_harness-darwin-universal"

echo "    -> Testing arm64 binary execution..."
"$RELEASE_DIR/my_harness-darwin-arm64" /help | head -n 5

echo "    -> Testing x64 binary execution via Rosetta..."
arch -x86_64 "$RELEASE_DIR/my_harness-darwin-x64" /help | head -n 5

echo "    -> Testing universal binary execution..."
"$RELEASE_DIR/my_harness-darwin-universal" /help | head -n 5
...
package_tarball() {
  ...
  rm -rf "$tmp_pack"
  mkdir -p "$tmp_pack"
  cp "$src_bin" "$tmp_pack/my_harness"
  chmod 755 "$tmp_pack/my_harness"
  tar -czf "$RELEASE_DIR/$target_name.tar.gz" -C "$tmp_pack" my_harness
  rm -rf "$tmp_pack"
}

package_tarball "my_harness-darwin-arm64" "$RELEASE_DIR/my_harness-darwin-arm64"
package_tarball "my_harness-darwin-x64" "$RELEASE_DIR/my_harness-darwin-x64"
package_tarball "my_harness-darwin-universal" "$RELEASE_DIR/my_harness-darwin-universal"

cd "$RELEASE_DIR"
shasum -a 256 my_harness-darwin-* > SHA256SUMS.txt
